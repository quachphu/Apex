...
**Option A — localtunnel (no signup required, recommended):**
...
**Option B — ngrok (requires valid authtoken):**

```bash
# Fix auth first: go to https://dashboard.ngrok.com/get-started/your-authtoken
...
```bash
# Terminal 1
cd apex/backend && python -m uvicorn main:app --reload --port 8000

# Terminal 2
cd apex/frontend && npm run dev
...
```bash
chmod +x apex/start.sh && ./apex/start.sh
```
...
├── requirements.txt
├── Makefile
└── start.sh              # One-command startup with tunnel detection
```
